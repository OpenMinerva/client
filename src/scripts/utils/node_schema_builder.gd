...
static func _add_node_metadata(root: Node, schema_name: String) -> void:
	var _class = root.get_class()
	var _schema_entry = _schema[schema_name]
	...
	root.set_meta("scene_node", _schema_entry.hidden == false)
	return
