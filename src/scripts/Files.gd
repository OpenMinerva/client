...
const app_name: String = "Open Wound"

func log_file_exists() -> bool:
	var today_log_filename = get_today_log_file_name()
	var docs_path = OS.get_user_data_dir()
	var does_log_file_exist = FileAccess.file_exists("%s/logs/%s.%s" % [docs_path, app_name, today_log_filename])
...
func create_log_file() -> String:
	var today_log_filename = get_today_log_file_name()
	var docs_path = OS.get_user_data_dir()
	var path = "%s/logs/%s.%s" % [docs_path, app_name, today_log_filename]
