...
static var _is_private: bool = true
...
static func set_playercount(current: int = -1, max: int = -1) -> void:
...
static func set_world(privacy: String = "Private", name: String = "Unnamed") -> void:
	if privacy == "Public":
		DiscordRPC.small_image_text = "Public world"
		DiscordRPC.small_image = "public-1"
		DiscordRPC.details = "In '%s'" % name
